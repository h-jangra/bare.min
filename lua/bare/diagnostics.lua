...
local function collect(bufnr)
  ...
  for _, d in ipairs(diags) do
    ...
    table.insert(items, {
      bufnr = d.bufnr, path = path, lnum = d.lnum + 1, col = d.col,
      severity = d.severity, icon = sev.icon, hl = sev.hl,
...
local function open_picker(title, items)
  if #items == 0 then return vim.notify("No diagnostics found", vim.log.levels.INFO) end
  ...
  for _, h in ipairs(hls) do
    ...
    vim.api.nvim_buf_set_extmark(buf, ns, h.row, 1 + h.icon_len + 1, { end_col = 1 + h.icon_len + 1 + h.loc_len, hl_group = "Directory" })
  end
  ...
  for _, k in ipairs({ "q", "<Esc>" }) do
    vim.keymap.set("n", k, function() if vim.api.nvim_win_is_valid(win) then vim.api.nvim_win_close(win, true) end end, opts)
  end
end

function M.picker_workspace() open_picker("Workspace Diagnostics", collect(nil)) end
function M.picker_buffer() open_picker("Buffer Diagnostics", collect(vim.api.nvim_get_current_buf())) end

function M.quickfix()
  vim.diagnostic.setqflist({ open = false })
  if #vim.fn.getqflist() == 0 then return vim.notify("No workspace diagnostics", vim.log.levels.INFO) end
  vim.cmd("copen")
end

function M.loclist()
  vim.diagnostic.setloclist({ open = false })
  if #vim.fn.getloclist(0) == 0 then return vim.notify("No buffer diagnostics", vim.log.levels.INFO) end
  vim.cmd("lopen")
end
...
function M.next_error() M.next(vim.diagnostic.severity.ERROR) end
function M.prev_error() M.prev(vim.diagnostic.severity.ERROR) end
function M.next_warn() M.next(vim.diagnostic.severity.WARN) end
function M.prev_warn() M.prev(vim.diagnostic.severity.WARN) end
...
function M.copy_line()
  ...
  if #diags == 0 then return vim.notify("No diagnostics on this line", vim.log.levels.WARN) end
