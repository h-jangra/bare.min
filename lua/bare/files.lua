...
local function is_valid_target(win)
  if not valid_win(win) or win == state.win then return false end
...
local function confirm_unsaved(action)
  if not (valid_buf(state.buf) and vim.bo[state.buf].modified) then return true end
...
local function setup_hl()
  if git.set_hl then git.set_hl() end
...
local function get_file_icon(name, is_dir)
  if is_dir then return " ", "Directory" end
  if has_icons then
...
local function scan_dir(dir, show_hidden)
  ...
  local sort_fn = function(a, b) return a.name:lower() < b.name:lower() end
...
local function update_line_icons()
  if not valid_buf(state.buf) then return end
  ...
  local orig_by_name = {}
  for _, e in ipairs(state.entries) do orig_by_name[e.name] = e end
  ...
  for i, line in ipairs(lines) do
    ...
    if text ~= "" then
      ...
      local is_dir = text:sub(-1) == "/" or (orig and orig.is_dir) or
          (full and fn.isdirectory(full) == 1)
      local is_hidden = clean:sub(1, 1) == "."
      local icon, icon_hl = get_file_icon(clean, is_dir)
      if is_hidden then icon_hl = "BareFilesHidden" end
      ...
      if orig then opts.id = orig.id end
      api.nvim_buf_set_extmark(state.buf, ns_icons, i - 1, 0, opts)
      api.nvim_buf_set_extmark(state.buf, ns_hl, i - 1, 0, { end_row = i - 1, end_col = #line, hl_group = text_hl })
      if git_info then
        api.nvim_buf_set_extmark(state.buf, ns_hl, i - 1, 0,
...
local function confirm_actions(actions)
  ...
  local function add(header, list, fmt)
    if #list > 0 then
      ...
      for _, item in ipairs(list) do table.insert(lines, fmt(item)) end
    end
  end
  add("CREATE", actions.creates, function(c) return "  + " .. c.name .. (c.src and " (copy)" or "") end)
  add("RENAME", actions.renames, function(r) return string.format("  ~ %s -> %s", r.old_name, r.new_name) end)
  add("DELETE", actions.deletes, function(d) return "  - " .. d.name end)
...
local function get_target_window()
  ...
  if valid_win(state.win) then pcall(api.nvim_win_set_width, state.win, config.width) end
  return target
end
...
function M.save()
  ...
  for _, orig in ipairs(state.entries) do orig_by_name[orig.name] = orig end
  ...
  for i, line in ipairs(cur_lines) do
    ...
    if not line_used[i] and text ~= "" and text ~= "." and text ~= ".." then
      local marks = api.nvim_buf_get_extmarks(state.buf, ns_icons, { i - 1, 0 }, { i - 1, -1 }, { details = true })
      local orig
      ...
      if text ~= "" and text ~= "." and text ~= ".." then
        ...
        if not src and #clip_items > 0 then
          for _, item in ipairs(clip_items) do
            ...
            if item_src and (clean == clean_name(item.name) or (item_pattern and clean:match(item_pattern))) then
              src = item_src
              if item_stat and item_stat.type == "directory" then is_dir = true end
              break
            end
          end
        end
    ...
    if ok then
      for _, b in ipairs(api.nvim_list_bufs()) do
        if api.nvim_buf_is_valid(b) and api.nvim_buf_get_name(b) == r.old_path then
          ...
          api.nvim_buf_call(b, function() vim.cmd("silent! checktime") end)
        end
      end
    else
...
function M.render(dir, focus_name)
  ...
  if not valid_buf(state.buf) then
    ...
    api.nvim_create_autocmd("BufWriteCmd", {
      ...
      callback = function()
        M.save()
        if valid_buf(state.buf) then vim.bo[state.buf].modified = false end
      end,
    })
...
function M.open_entry(cmd)
  if not valid_buf(state.buf) or not confirm_unsaved("opening") then return end
...
function M.parent_dir()
  if not state.current_dir or not confirm_unsaved("navigating") then return end
...
function M.setup_buffer_keymaps(buf)
  ...
  nmap({ "<CR>", "l" }, function() M.open_entry("edit") end, "Open / Enter")
  nmap({ "h", "-" }, M.parent_dir, "Parent directory")
  nmap({ "v", "<C-v>" }, function() M.open_entry("vsplit") end, "Open in vertical split")
  nmap({ "s", "<C-x>" }, function() M.open_entry("split") end, "Open in horizontal split")
  nmap("<C-t>", function() M.open_entry("tabedit") end, "Open in new tab")
  ...
  for _, k in ipairs({ "y", "yy" }) do
    nmap(k, function() return yank_entry(k, false) end, "Yank entry", { expr = true })
  end
  nmap("p", function() M.paste(false) end, "Paste after")
  nmap("P", function() M.paste(true) end, "Paste before")
  nmap({ "g.", "H" }, M.toggle_hidden, "Toggle hidden files")
  nmap("R", function() M.render(state.current_dir) end, "Refresh")
  nmap("~", function() M.render(fn.getcwd()) end, "Go to cwd")
  nmap({ "q", "<Esc>" }, M.close, "Close sidebar")

  -- Visual mode mappings for multifile ops
  xmap({ "<CR>", "l" }, function() open_visual("edit") end, "Open selected entries")
  xmap({ "v", "<C-v>" }, function() open_visual("vsplit") end, "Open selected in vertical splits")
  xmap({ "s", "<C-x>" }, function() open_visual("split") end, "Open selected in horizontal splits")
  xmap("<C-t>", function() open_visual("tabedit") end, "Open selected in new tabs")
  for _, k in ipairs({ "y", "Y" }) do
    xmap(k, function() return yank_entry("y", true) end, "Yank selected entries", { expr = true })
  end
  xmap("p", function() M.paste(false) end, "Paste after")
  xmap("P", function() M.paste(true) end, "Paste before")
end
...
function M.open(dir)
  ...
  if valid_win(state.win) then
    api.nvim_set_current_win(state.win)
    if dir ~= state.current_dir then M.render(dir) end
    return
  end
...
function M.close()
  if not valid_win(state.win) then return end
...
function M.setup(opts)
  ...
  api.nvim_create_autocmd("WinEnter", {
    ...
    callback = function()
      ...
      if is_valid_target(win) then state.target_win = win end
    end,
  })
