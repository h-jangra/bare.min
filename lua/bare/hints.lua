...
local function get_mappings(mode)
  ...
  for _, m in ipairs(vim.api.nvim_buf_get_keymap(0, mode)) do
    if m.desc and m.desc ~= "" then
      maps[term(m.lhs)] = { desc = m.desc, map = m }
    end
  end
  for _, m in ipairs(vim.api.nvim_get_keymap(mode)) do
    local lhs = term(m.lhs)
    if not maps[lhs] and m.desc and m.desc ~= "" then
      maps[lhs] = { desc = m.desc, map = m }
    end
  end
  for k, desc in pairs(builtin) do
    local lhs = term(k)
    if not maps[lhs] then
      maps[lhs] = { desc = desc }
    end
  end
  return maps
...
function M.query(trigger, mode)
  ...
  local clues = collect_clues(query, maps)
  if #clues == 0 then
    return vim.api.nvim_feedkeys(query, "n", false)
  end

  state.is_running = true
  state.timer = vim.uv.new_timer()
  state.timer:start(20, 0,
...
function M.setup(opts)
  ...
  for _, trig in ipairs(triggers) do
    for _, mode in ipairs({ "n", "x" }) do
      vim.keymap.set(mode, trig, function()
        M.query(trig, mode)
      end, { noremap = true, silent = true, desc = "Hints" })
    end
  end
