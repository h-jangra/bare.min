...
local function get_word()
	local line, col = vim.api.nvim_get_current_line(), vim.fn.col(".")
	if not line:sub(col, col):match("%w") then
		return nil
	end
...
function M.setup()
	...
	map("x", "sa", function()
		...
		local keys =
			vim.api.nvim_replace_termcodes("<Esc>`>a" .. close .. "<Esc>`<i" .. open .. "<Esc>", true, false, true)
		vim.api.nvim_feedkeys(keys, "n", false)
	end)
	for lhs, fn in pairs({ sa = M.add, sd = M.delete, sc = M.change, sr = M.change }) do
		map("n", lhs, function()
			fn(vim.fn.getcharstr())
		end)
	end
end
